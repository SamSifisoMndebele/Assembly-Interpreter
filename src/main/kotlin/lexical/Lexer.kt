...
abstract class Lexer(source: String) {
    companion object {
        ...
        private val binPattern = Pattern.compile("^(?:0b[01]+|[01]+[by])", Pattern.CASE_INSENSITIVE)
        private val octPattern = Pattern.compile("^(?:0[qo][0-7]+|[0-7]+[qo])", Pattern.CASE_INSENSITIVE)
        private val decPattern = Pattern.compile("^[-+]?[0-9]+(?:_[0-9]+)*[dt]?", Pattern.CASE_INSENSITIVE)
//        private val encRealPattern = Pattern.compile("^[-+]?[0-9]+(?:_[0-9]+)*r", Pattern.CASE_INSENSITIVE)
